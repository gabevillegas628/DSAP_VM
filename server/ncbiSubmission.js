...
const formatNCBIDate = (dateString) => {
    ...
    // NCBI accepts: DD-Mmm-YYYY, Mmm-YYYY, or YYYY
...
const formatNCBICountry = (country) => {
  // NCBI format is "Country: State/Province, Location"
  // For USA, it should be "USA: State"
  if (!country) return null;
  
  // Basic validation - just return if it looks reasonable
  return country.trim();
};
...
const generateFeatureTable = (sequences, outputPath) => {
    ...
    const tableContent = sequences.map(seq => {
        ...
        if (seq.country) {
            formattedCountry = formatNCBICountry(seq.country);
