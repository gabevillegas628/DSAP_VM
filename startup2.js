...
// Update backend .env file with frontend URL
function updateBackendEnv(frontendUrl) {
  try {
    const envFile = path.join(process.cwd(), 'server', '.env');

    if (!fs.existsSync(envFile)) {
      logError(`Environment file doesn't exist: ${envFile}`);
      return false;
    }

    let content = fs.readFileSync(envFile, 'utf8');
    const lines = content.split('\n');
    let frontendUrlUpdated = false;

    // Update existing FRONTEND_URL line or add it
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].startsWith('FRONTEND_URL=')) {
        lines[i] = `FRONTEND_URL=${frontendUrl}`;
        frontendUrlUpdated = true;
        break;
      }
    }

    // If FRONTEND_URL wasn't found, add it
    if (!frontendUrlUpdated) {
      lines.push(`FRONTEND_URL=${frontendUrl}`);
    }

    // Write back to file
    fs.writeFileSync(envFile, lines.join('\n'));
    success(`Backend .env updated: FRONTEND_URL=${frontendUrl}`);
    return true;
  } catch (error) {
    logError(`Failed to update backend .env: ${error.message}`);
    return false;
  }
}
...
async function startup() {
  try {
    ...
    const envUpdated = updateBackendEnv(urls.frontend);

    if (!configUpdated || !corsUpdated || !envUpdated) {
